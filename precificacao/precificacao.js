...
const custosIndiretosPredefinidosBase = [ // This is the base template, never modified
...
function atualizarTabelaCustosIndiretos() {
  ...
  //  const custosFiltrados = custosExibicao.filter(custo => custo.descricao.toLowerCase().includes(termoBusca)); //termoBusca não existe neste escopo.

   custosExibicao.forEach((custo) => { //Usa custosExibicao, não custosFiltrados
...
//Zerar Custo Indireto

function zerarCustoIndireto(index, tipo) {
    if (tipo === 'predefinido') {
        // Encontra a descrição do custo predefinido
        const descricao = custosIndiretosPredefinidosBase[index].descricao;
        // Encontra o custo predefinido correspondente na lista de trabalho
        const custo = custosIndiretosPredefinidos.find(c => c.descricao === descricao);

        if (custo) {
            custo.valorMensal = 0; // Zera o valor
            // Atualiza o campo de input correspondente na lista
            const input = document.getElementById(`custo-indireto-${index}`);
            if (input) {
                input.value = "0.00";
            }
        }
        ...
        // Encontra o custo adicional pelo tempIndex e zera
        const custoIndex = custosIndiretosAdicionais.findIndex(c => c.tempIndex === index);
        if (custoIndex !== -1) {
          custosIndiretosAdicionais[custoIndex].valorMensal = 0;
        }
    }

    atualizarTabelaCustosIndiretos();
    calcularCustos(); // Recalcula os custos totais
    salvarDadosFirebase();
}
...
function cadastrarProduto() {
    ...
    for (let i = 0; i < linhasMateriais.length; i++) {
        ...
        let quantidadeMaterial = quantidadeInput ? parseFloat(inputQuantidade.value) : 0;
